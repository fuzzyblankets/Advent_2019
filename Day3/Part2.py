...
def move_right (move, path_count):
    end_location = int(move[1:])
    for i in range(0,end_location):
        path[0] += 1
        path[2] += 1
        path_list[path_count].append(path[:])

def move_left (move, path_count):
    end_location = int(move[1:])
    for i in range(0,end_location):
        path[0] -= 1
        path[2] += 1
...
for instruction in instructions:
    ...
    for move in instruction:
        if move[0] == "R":
            move_right(move,path_count)
        elif move[0] == "L":
            move_left(move,path_count)
        elif move[0] == "U":
            move_up(move,path_count)
        elif move[0] == "D":
            move_down(move,path_count)
        else:
            raise ValueError("Bad instruction")
    path_count+=1

step_list = []
path_list_strip = [[i[0], i[1]] for i in path_list[1]]
for step in path_list[0]:
    step_coords = [step[0], step[1]]
    if step_coords in path_list_strip:
        index = path_list_strip.index(step_coords)
        step_count_1 = step[2]
        step_count_2 = path_list[1][index][2]
        step_count = step_count_1 + step_count_2
        step_list.append(step_count)


fewest_steps = min(step_list)
print(fewest_steps)
